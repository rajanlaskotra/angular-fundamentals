...
export class EventService {
  ...
  //this method add & update event, the custom server has endpoint '/api/events', which add event when id doesn't exist & 
  //update(here i.e add session) when id exists. thats why using PUT not used for update.
  saveEvent(event) {
    let options = { headers: new HttpHeaders({'Content-Type': 'application/json'}) }

    return this.http.post<IEvent>('/api/events',event, options)     //server returns the new added event with 'id' being set.
      .pipe(catchError(this.handleError<IEvent>('saveEvent')))      
  }

  searchSession(searchTerm: string): Observable<ISession[]> {
    return this.http.get<ISession[]>('/api/sessions/search?search=' + searchTerm)       //QueryString
...
//getting event data from our custom server. (ngf-server)
...
//     //     country: 'Netherlands'
//     //   },
//       onlineUrl: 'http://ng-nl/angular.nl/',
//       sessions: [
//         {
//           id: 1,
